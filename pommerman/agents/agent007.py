...
class Agent007(BaseAgent):
    ...
    def reset(self, ammo=1, is_alive=True, blast_strength=None, can_kick=False):
        self._character.position = self.start_position
        self._character.ammo = ammo
        self._character.is_alive = is_alive
        self._character.blast_strength = blast_strength or constants.DEFAULT_BLAST_STRENGTH
        self._character.can_kick = can_kick
        self.ongoing_attack = False
        self.old_message = [0, 0]
        self._prev_direction = None
        self.role = 0  # 0: unknown, 1: leader, 2: assistant
            ...
            if self.one_on_one:
                ...
                if self.one_on_one_steps > 3:
                    if self._maybe_bomb(ammo, blast_strength, items, dist, my_position, directions):
                        self.one_on_one_steps = 0
                        # print("placed bomb in one on one situation")
                        return [constants.Action.Bomb.value] + message
                else:
                    self.one_on_one_steps += 1
                    # print(self.one_on_one_steps)
            else:
                self.check_one_on_one(obs)
    ...
    def communicator(self, obs):
        ...
        if self.role != 0 and received == (8, 8) and not self.ongoing_attack:
            self.ongoing_attack = True
        elif self.role != 0 and received == (8, 8):
            self.ongoing_attack = False

        # end attack if teammate died
        if self.ongoing_attack and received == (0, 0):
            self.ongoing_attack = False
        ...
        if not self.role:
            ...
            # attack mode
                ...
                if self.role == 1:
                    return_to_normal = self.return_to_normal(obs)
                    if return_to_normal:
                        self.ongoing_attack = False
                    message = self.get_attack_message(obs, return_to_normal)
                else:
                    message = self.get_attack_message(obs, False)
    ...
    def should_we_attack(self, obs):
        ...
        # to cut the board is not necessary due to low visibility
        # board = board[max(0, my_pos[0] - 9):min(10, my_pos[0] + 10), max(0, my_pos[1] - 9):min(10, my_pos[1] + 10)]
        # print("teammate: " + str(teammate in board) + "\t" + " enemy: " + str((enemies[0] in board or enemies[1] in board)))

        if teammate in board and (enemies[0] in board or enemies[1] in board):
            return True

        return False

    def get_attack_message(self, obs, return_to_normal):

        if return_to_normal:
            return [8, 8]
        else:
            return [obs['position'][0]//2 + 2, obs['position'][1]//2 + 2]
    ...
    def return_to_normal(self, obs):
        ...
        if teammate not in board or not (enemies[0] in board or enemies[1] in board):
            if self.steps_without_contact >= 5:
                self.steps_without_contact = 0
                return True
            else:
    ...
    def check_one_on_one(self, obs):
        ...
        # pos = obs['position']
        #
        # if teammate not in board:
        #     board = board[max(pos[0] - 1, 0):min(pos[0] + 2, 11), max(pos[1] - 1, 0):min(pos[1] + 2, 11)]
        #     if enemies[0] in board or enemies[1] in board:
