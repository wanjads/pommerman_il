...
class Leif(agents.BaseAgent):
    ...
    def act(self, obs, action_space):
        ...
        obj = utils.obsToPlanes(constants.DODGE_BOARD_SIZE)
        planes = obj.planeFilling(obs, obj.planes)
        #last_hn, last_cn = self.hidden[-1][0], self.hidden[-1][1]
        obs = torch.from_numpy(planes).float().to(self.model.device)
        #print(self.model.device)

        with torch.no_grad():
            self.model.eval()
            #last_hn, last_cn = torch.tensor(last_hn).unsqueeze(0), torch.tensor(last_cn).unsqueeze(0)
            val, probs, hn, cn = self.model(obs.unsqueeze(0))
        #    unsequeezed = obs.unsqueeze(0)
        #    val, probs = self.model(unsequeezed)
    ...
    def clear(self):
        ...
        return self.states, self.actions, self.probs, self.values # self.hidden,
